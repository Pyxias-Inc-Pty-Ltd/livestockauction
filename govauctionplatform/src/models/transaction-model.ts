    ...
    if (this.$locals.isPaymentStatusModified) {
      ...
      switch (doc.transactionType) {
        case "RESERVATION":
          ...
          notificationMessage = `Your payment for reservation of "${(item as any).title.en}" has ${doc.status === EPaymentStatus.COMPLETED ? 'been completed successfully' : 'failed'}.`;
          htmlContent = doc.status === EPaymentStatus.COMPLETED ? reservePricePaymentTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title.en) : reservePricePaymentFailedTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title.en);
          break;
        case "PURCHASE":
          ...
          notificationMessage = `Your payment for purchase of "${(item as any).title.en}" has ${doc.status === EPaymentStatus.COMPLETED ? 'been completed successfully' : 'failed'}.`;
          htmlContent = doc.status === EPaymentStatus.COMPLETED ? purchasePaymentTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title.en) : purchasePaymentFailedTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title.en);
          break;
        default: // Assumes REFUND
          ...
          notificationMessage = `Your refund for "${(item as any).title.en}" has ${doc.status === EPaymentStatus.COMPLETED ? 'been completed successfully' : 'failed'}.`;
          htmlContent = doc.status === EPaymentStatus.COMPLETED ? refundTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title.en) : refundFailedTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title.en);
          break;
      }
