    ...
    if (this.$locals.isPaymentStatusModified) {
      ...
      switch (doc.transactionType) {
        case "RESERVATION":
          ...
          htmlContent = doc.status === EPaymentStatus.COMPLETED ? reservePricePaymentTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title) : reservePricePaymentFailedTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title);
          break;
        case "PURCHASE":
          ...
          htmlContent = doc.status === EPaymentStatus.COMPLETED ? purchasePaymentTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title) : purchasePaymentFailedTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title);
          break;
        default: // Assumes REFUND
          ...
          htmlContent = doc.status === EPaymentStatus.COMPLETED ? refundTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title) : refundFailedTemplate.replace('[UserName]', (buyer as IBidder).isOrganization ? (buyer as IBidder).name as string : (buyer as IBidder).firstName as string).replace('[ItemName]', (item as IItem).title);
          break;
      }
