...
import { createAuctionSchema, updateAuctionSchema } from '../shared/auction-validation';
...
import { EAuctionStatus, EAuctionSortType, EPermission, ESortOrderType, EPublishedStatus, EAttachmentType } from '../globals';
...
router.post(p.createAuction, requirePermission(EPermission.AUCTION_CREATE), async (req: Request, res: Response) => {
  try {
    ...
    Joi.assert(req.body, createAuctionSchema);
...
router.put(p.updateAuction, SellerOnly(), async (req: Request, res: Response) => {
  try {
    Joi.assert(req.body, updateAuctionSchema);
